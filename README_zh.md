...
| `dimension_translation_mode` | `Literal['mcdr', 'minecraft']` | `'minecraft'`           | `mcdr`: 维度名称由MCDR翻译; `minecraft`: 维度名称由Minecraft翻译。在1.19及以上版本，用`mcdr`项可以避免因翻译键名改动造成的翻译失败 |
| `custom_dimension_name`      | `Dict[str, Dict[str, str]`     | 过长无法展示                  | 维度名称的翻译键值映射。首层键名为语言，二级键名为维度ID（支持非原版维度，原版维度必须填写且应去掉命名空间） |

上述提到的含固定键值对的的配置项如下:

**上述星标项的配置内容：**

| `command_prefix` 的键 | Value type | Default value             | Introduction           |
| --------------------- | ---------- | ------------------------- | ---------------------- |
| `where_is`            | `str`      | `['!!vris', '!!whereis']` | 查询玩家坐标的指令前缀 |
| `here`                | `str`      | `['!!here']`              | 广播自己坐标的指令前缀 |

| `highlight_time` 的键 | Value type | Default value | Introduction                                 |
| --------------------- | ---------- | ------------- | -------------------------------------------- |
